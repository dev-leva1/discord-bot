...
COMMANDS_TOTAL = Counter(
    "bot_commands_total", "Total commands processed", ["command", "guild_id", "success"]
)
...
MESSAGES_PROCESSED = Counter("bot_messages_processed", "Total messages processed", ["guild_id"])
...
ACTIVE_USERS = Gauge("bot_active_users", "Number of active users", ["guild_id"])
GUILDS_COUNT = Gauge("bot_guilds_count", "Number of guilds the bot is connected to")
ERRORS_COUNT = Counter("bot_errors_total", "Total errors encountered", ["type", "module"])
AUTOMOD_ACTIONS = Counter(
    "bot_automod_actions_total", "Total automod actions taken", ["action_type", "guild_id"]
)
...
DB_OPERATIONS = Counter(
    "bot_db_operations_total", "Total database operations performed", ["operation", "table"]
)
...
VOICE_CONNECTIONS = Gauge("bot_voice_connections", "Number of active voice connections")
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            if SENTRY_ENABLED:
                ...
                sentry_sdk.capture_exception(error=e, extra=extra_data)

            # Логирование ошибки
            logger.error(f"Ошибка при выполнении команды {command_name}: {str(e)}", exc_info=True)
...
def capture_error(error: Exception, context: Optional[Dict[str, Any]] = None) -> None:
    ...
    try:
        ...
        print(f"Критическая ошибка при логировании исключения: {e}", f"Исходная ошибка: {error}")
