...
class Welcome:
    ...
    async def setup(self):
        @self.bot.tree.command(name="setwelcome", description="Установить канал для приветствий")
        @app_commands.checks.has_permissions(administrator=True)
        async def setwelcome(interaction: discord.Interaction, channel: discord.TextChannel):
            if not isinstance(channel, discord.TextChannel):
                await interaction.response.send_message(
                    "Пожалуйста, выберите текстовый канал!", ephemeral=True
                )
                return
            ...
            if not channel.permissions_for(interaction.guild.me).send_messages:
                await interaction.response.send_message(
                    "У меня нет прав для отправки сообщений в этот канал!", ephemeral=True
                )
                return

            if not channel.permissions_for(interaction.guild.me).embed_links:
                await interaction.response.send_message(
                    "У меня нет прав для отправки эмбедов в этот канал!", ephemeral=True
                )
                return
