...
class WarningSystem(commands.Cog, WarningsServiceContract):
    ...
    @app_commands.describe(
        member="Участник, которому нужно выдать предупреждение", reason="Причина предупреждения"
    )
    async def add_warning(
        ...
        if member.bot:
            ...
            return await (ctx.response.send_message if is_interaction else ctx.send)(
        ...
        embed = discord.Embed(
            title="Выдано предупреждение", color=discord.Color.orange(), timestamp=datetime.utcnow()
        )
        ...
        for threshold, punishment in sorted(
            self.config["punishments"].items(), key=lambda x: int(x[0])
        ):
                ...
                if punishment == "mute_1h":
                    try:
                        await member.timeout(
                            timedelta(hours=1),
                            reason=f"Автоматический мут: {warning_count} предупреждений",
                        )
                        await send_followup(
                            f"{member.mention} получает мут на 1 час за {warning_count} предупреждений"
                        )
                    ...
                    try:
                        ...
                        await send_followup(
                            f"{member.mention} получает мут на 12 часов за {warning_count} предупреждений"
                        )
                    except discord.Forbidden:
                        await send_followup("Не удалось выдать мут - недостаточно прав")
                    ...
                    try:
                        await member.kick(
                            reason=f"Автоматический кик: {warning_count} предупреждений"
                        )
                        await send_followup(
                            f"{member.mention} был кикнут за {warning_count} предупреждений"
                        )
                    except discord.Forbidden:
                    ...
                    try:
                        await member.ban(
                            reason=f"Автоматический бан: {warning_count} предупреждений"
                        )
                        await send_followup(
                            f"{member.mention} был забанен за {warning_count} предупреждений"
                        )
                    except discord.Forbidden:
    ...
    async def remove_warning(
        self, ctx: Union[commands.Context, discord.Interaction], member: discord.Member, index: int
    ):
        ...
        await (ctx.response.send_message if is_interaction else ctx.send)(
            f'Предупреждение #{index} было удалено у {member.mention}: "{removed["reason"]}"'
        )

    @commands.hybrid_command(
        name="warn_list", description="Показать список предупреждений участника"
    )
    @app_commands.describe(member="Участник, чьи предупреждения нужно показать")
    async def list_warnings(
        self, ctx: Union[commands.Context, discord.Interaction], member: discord.Member
    ):
        ...
        embed = discord.Embed(
            title=f"Предупреждения | {member.display_name}", color=discord.Color.blue()
        )
