...
class RoleRewards:
    def __init__(self, bot):
        self.bot = bot
        self.config_file = Path("role_rewards.json")
    ...
    async def setup(self):
        @self.bot.tree.command(
            name="addrole",
            description="Добавить роль-награду за достижение определенного уровня"
        )
        ...
        @self.bot.tree.command(
            name="removerole",
            description="Удалить роль-награду за уровень"
        )
        @app_commands.checks.has_permissions(manage_roles=True)
        async def removerole(
            ...
            if guild_id not in self.config or str(level) not in self.config[guild_id]:
                await interaction.response.send_message(
                    "Для этого уровня не настроена роль-награда!",
                    ephemeral=True
                )
                return
        ...
        @self.bot.tree.command(
            name="listroles",
            description="Показать список ролей-наград за уровни"
        )
        async def listroles(interaction: discord.Interaction):
            ...
            if guild_id not in self.config or not self.config[guild_id]:
                await interaction.response.send_message(
                    "На сервере нет настроенных ролей-наград!",
                    ephemeral=True
                )
                return
